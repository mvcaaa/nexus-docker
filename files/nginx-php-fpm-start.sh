...
if [ -z "$SKIP_COMPOSER" ]; then
    ...
    if [ -f "$COMPOSER_WORKING_DIR/composer.lock" ]; then
        if [ "$APPLICATION_ENV" == "development" ]; then
            composer global require hirak/prestissimo
            composer install --prefer-dist --dev --verbose --optimize-autoloader --working-dir=$COMPOSER_WORKING_DIR
        else
            composer global require hirak/prestissimo
            composer install --no-dev --working-dir=$COMPOSER_WORKING_DIR
        fi
    fi
fi
